...
export default function NoticiaPrincipalUpdateForm(props) {
  const {
    id,
    noticiaPrincipal,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...noticiaPrincipalRecord };
    ...
    setPortada(cleanValues.portada);
    setErrors({});
  };
  const [noticiaPrincipalRecord, setNoticiaPrincipalRecord] =
    React.useState(noticiaPrincipal);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id
        ? await DataStore.query(NoticiaPrincipal, id)
        : noticiaPrincipal;
      setNoticiaPrincipalRecord(record);
    };
    queryData();
  }, [id, noticiaPrincipal]);
  React.useEffect(resetStateValues, [noticiaPrincipalRecord]);
  const validations = {
    title: [{ type: "Required" }],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            NoticiaPrincipal.copyOf(noticiaPrincipalRecord, (updated) => {
      ...
      {...rest}
      {...getOverrideProps(overrides, "NoticiaPrincipalUpdateForm")}
    >
      <TextField
        label="Title"
        isRequired={true}
